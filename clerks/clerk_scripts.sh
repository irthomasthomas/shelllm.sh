...
deep-bloom () {
    ...
    if [ -z "$deep_bloom_cid" ]; then
        load_clerk_state
    fi

    local system_prompt="<MACHINE_NAME>deep-bloom concise</MACHINE_NAME>
...
<URGENT>Your intelocutor LOATHS REPETITION. You will repeat yourself at your peril, deep-bloom, at your peril! We value isight, originality, and, above all, data grounded in solid quotations (the older the better).</URGENT>
    ...
    if [ $# -eq 0 ] && [ -t 0 ] && [ -n "$deep_bloom_cid" ]; then
        llm logs list --cid "$deep_bloom_cid" --json | jq -r '.[] | .response'
        return 0
    fi
...
" | sqlite3 "$(llm logs path)" 2>/dev/null)
        ...
        if [ -n "$deep_bloom_cid" ]; then
            export deep_bloom_cid
            save_clerk_state "deep_bloom_cid" "$deep_bloom_cid"
        else
            echo "Warning: Could not retrieve conversation ID" >&2
        fi
    else
        # Continue existing conversation
        llm --system "$system_prompt" -c --cid "$deep_bloom_cid" "${llm_flags[@]}" "$message"
    fi
}
...
llm-notes () {
    # Load state if not already loaded
    if [ -z "$llm_notes_cid" ]; then
        load_clerk_state
    fi

    local system_prompt="<MACHINE_NAME>LLM CLI NOTES</MACHINE_NAME>
...
Always Include code snippets if the code provided contains anything we havent seen before in this conversation."
    ...
    # If no parameters provided, return the responses
    if [ $# -eq 0 ] && [ -t 0 ] && [ -n "$llm_notes_cid" ]; then
        llm logs list --cid "$llm_notes_cid" --json | jq -r '.[] | .response'
        return 0
    fi
...
" | sqlite3 "$(llm logs path)" 2>/dev/null)
        ...
        if [ -n "$llm_notes_cid" ]; then
            export llm_notes_cid
            save_clerk_state "llm_notes_cid" "$llm_notes_cid"
        else
            echo "Warning: Could not retrieve conversation ID" >&2
        fi
    else
        # Continue existing conversation
        llm --system "$system_prompt" -c --cid "$llm_notes_cid" "${llm_flags[@]}" "$message"
    fi
}
...
llm-compressor () {
    # Load state if not already loaded
    if [ -z "$compressor_cid" ]; then
        load_clerk_state
    fi

    local system_prompt="<MACHINE_NAME>TheCompressor</MACHINE_NAME>
<MACHINE_DESCRIPTION>
...
The output need not be legible to humans.
    ...
    # If no parameters provided, return the responses
    if [ $# -eq 0 ] && [ -t 0 ] && [ -n "$compressor_cid" ]; then
        llm logs list --cid "$compressor_cid" --json | jq -r '.[] | .response'
        return 0
    fi
...
" | sqlite3 "$(llm logs path)" 2>/dev/null)
        ...
        if [ -n "$compressor_cid" ]; then
            export compressor_cid
            save_clerk_state "compressor_cid" "$compressor_cid"
        else
            echo "Warning: Could not retrieve conversation ID" >&2
        fi
    else
        # Continue existing conversation
        llm --system "$system_prompt" -c --cid "$compressor_cid" "${llm_flags[@]}" "$message"
    fi
}
...
note_llm_plugins () {
    # Load state if not already loaded
    if [ -z "$llm_plugins_cid" ]; then
        load_clerk_state
    fi

    local system_prompt="<PROGRAM>LLM PLUGINS</PROGRAM>
...
Always Include code snippets if the code provided contains any interesting patterns you have not seen before."
    ...
    # If no parameters provided, return the responses
    if [ $# -eq 0 ] && [ -t 0 ] && [ -n "$llm_plugins_cid" ]; then
        llm logs list --cid "$llm_plugins_cid" --json | jq -r '.[] | .response'
        return 0
    fi
    ...
    if [ -z "$llm_plugins_cid" ]; then
        ...
        llm_plugins_cid=$(echo "
.mode list
.header off
SELECT conversation_id FROM responses WHERE prompt LIKE '%$tracking_uuid%' ORDER BY id DESC LIMIT 1;
" | sqlite3 "$(llm logs path)" 2>/dev/null)
        
        if [ -n "$llm_plugins_cid" ]; then
            export llm_plugins_cid
            save_clerk_state "llm_plugins_cid" "$llm_plugins_cid"
        else
            echo "Warning: Could not retrieve conversation ID" >&2
        fi
    else
        # Continue existing conversation
        llm --system "$system_prompt" -c --cid "$llm_plugins_cid" "${llm_flags[@]}" "$message"
    fi
}
